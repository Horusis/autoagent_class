...
def load_images(folder_or_list, size, square_ok=False, 
    ...
    # len_postfix = len(os.path.splitext(folder_content[0])[1])
    # print(f'>> Sorting images by number in name, with postfix length {len_postfix}')
    ...
    for name in folder_content:
        ...
        dot_index = len(basename) - len_postfix
        number_start = 0
        number_end = 0
        ...
        for i in range(dot_index-1, 0, -1):
            if basename[i].isdigit():
                number_end = i + 1
                break
        print(f' - image name: {basename}, number extracted: {basename[number_start:number_end]}')
        img_numbers.append(float(basename[number_start:number_end]))
