...
class _PanopticPrediction:
    ...
    def __init__(self, segmentation, segments_info, metadata=None):
        if segments_info is None:
            ...
            segments_info = []
            for panoptic_label in np.unique(segmentation.numpy()):
                if panoptic_label == -1:
                    # VOID region.
                    continue
        ...
        segment_ids, areas = torch.unique(segmentation, sorted=True, return_counts=True)
        areas = areas.numpy()
    ...
    if classes is not None:
        if class_names is not None and len(class_names) > 0:
            if isinstance(class_names, dict):
                labels = [class_names.get(i, str(i)) for i in classes]
            else:
...
class Visualizer:
    ...
    def __init__(self, metadata=None, scale=1.0, instance_mode=ColorMode.SEGMENTATION, font_size_scale=1.0, **kwargs):
        """
        Args:
        ...
        self.metadata = metadata if metadata is not None else MetadataCatalog.get("default")
        self.scale = scale
        self.font_size_scale = font_size_scale
        self._instance_mode = instance_mode
        self.keypoint_threshold = _KEYPOINT_THRESHOLD

    def set_image(self, img_rgb, aux_img_rgb=None):
        self.img = np.asarray(img_rgb).clip(0, 255).astype(np.uint8)
        self.output = VisImage(self.img, scale=self.scale)
        self.aux_output = VisImage(self.img, scale=self.scale)

        self.aux_img = self.img
        if aux_img_rgb is not None:
            self.aux_img = np.asarray(aux_img_rgb).clip(0, 255).astype(np.uint8)
            self.aux_output = VisImage(self.aux_img, scale=self.scale)

        # too small texts are useless, therefore clamp to 9
        self._default_font_size = (
            max(np.sqrt(self.output.height * self.output.width) // 90, 10 // self.scale) * self.font_size_scale
        )
    ...
    def draw_pan_seg(self, segmentation, segments_info=None, area_threshold=None, alpha=0.7, **kwargs):
        """
        Draw panoptic prediction annotations or results.

        Args:
            segmentation (Tensor): of shape (height, width) where the values are ids for each
                segment.
            segments_info (list[dict] or None): Describe each segment in `segmentation`.
        ...
        if self._instance_mode == ColorMode.IMAGE_BW:
            self.output.reset_image(self._create_grayscale_image(self.img, pred.non_empty_mask()))
            self.aux_output.reset_image(self._create_grayscale_image(self.aux_img, pred.non_empty_mask()))
    ...
    def draw_sem_seg(self, segmentation, area_threshold=None, alpha=0.8, **kwargs):
        ...
        Args:
            segmentation (Tensor or ndarray): the segmentation of shape (H, W).
    ...
    def draw_ins_seg(self, instances, jittering: bool = True, **kwargs):
        ...
        Args:
            instances (Instances): the output of an instance detection/segmentation
        ...
        if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get("thing_colors"):
            colors = (
                [self._jitter([x / 255 for x in self.metadata.thing_colors.get(c, (0, 0, 0))]) for c in classes]
                if jittering
                else [
                    tuple(mplc.to_rgb([x / 255 for x in self.metadata.thing_colors.get(c, (0, 0, 0))]))
                    for c in classes
                ]
            )
    ...
    def draw_ref_seg(self, segmentation, segments_info, phrases=[], area_threshold=None, alpha=0.8, **kwargs):
        ...
        Returns:
            output (VisImage): image object with visualizations.
        """
        if len(phrases) == 0 or phrases is None:
            phrases = [""]
        if isinstance(segmentation, torch.Tensor):
            segmentation = segmentation.numpy()
        labels, areas = np.unique(segmentation, return_counts=True)
    ...
    def draw_vgd_seg(self, instances, vprompt_masks=None, jittering: bool = True, area_threshold=None, **kwargs):
        ...
        scores = instances.scores if instances.has("scores") else None
        classes = instances.pred_classes.tolist() if instances.has("pred_classes") else None
        return_contiguous_labels = kwargs.pop("return_contiguous_labels", False)
        if return_contiguous_labels:
            sampled_labels = kwargs.pop("sampled_labels", None)
            labels = _create_text_labels(classes, scores, self.metadata.get("thing_classes", None))
        else:
            num_classes = kwargs.pop("num_classes", 1)
            sampled_labels = sorted(list(range(num_classes)))
        ...
        for i, (sampled_label, vprompt_mask) in enumerate(zip(sampled_labels, vprompt_masks)):
            if isinstance(vprompt_mask, torch.Tensor):
                vprompt_mask = vprompt_mask.numpy()
            binary_mask = (vprompt_mask == 1).astype(np.uint8)
            ...
            if self.metadata.get("thing_classes", None) is not None and return_contiguous_labels:
                text = f"{self.metadata.thing_classes[sampled_label]}"
            else:
                text = f"class{i}"
    ...
    def draw_inter_seg(
        ...
        sorted_idxs = np.argsort(-areas).tolist()
        labels = labels[sorted_idxs]
        scores = [segments_info["score"]]
        classes = [segments_info["category_id"]]

        return_contiguous_labels = kwargs.pop("return_contiguous_labels", False)
        if return_contiguous_labels:
            sampled_labels = kwargs.pop("sampled_labels", None)
            texts = _create_text_labels(classes, scores, self.metadata.get("thing_classes", None))
        else:
            num_classes = kwargs.pop("num_classes", 1)
            sampled_labels = sorted(list(range(num_classes)))
            class_names = [f"class{i}" for i in range(num_classes)]
            texts = _create_text_labels(classes, scores, class_names)

        if self.metadata.get("thing_colors", None) is not None:
            colors = [tuple(mplc.to_rgb([x / 255 for x in self.metadata.thing_colors[c]])) for c in classes]
            alpha = 0.8
        else:
            colors = None
            alpha = 0.5
        ...
        for sampled_label, vprompt_mask in zip(sampled_labels, vprompt_masks):
            if isinstance(vprompt_mask, torch.Tensor):
                vprompt_mask = vprompt_mask.numpy()
            binary_mask = (vprompt_mask == 1).astype(np.uint8)
    ...
    def draw_dataset_dict(self, dic):
        ...
        if sem_seg is None and "sem_seg_file_name" in dic:
            with open(dic["sem_seg_file_name"], "rb") as f:
    ...
    def draw_text(
        ...
        *,
        ...
        x, y = position
        output_image.ax.text(
            x,
            y,
            text,
            size=font_size * output_image.scale,
            family="sans-serif",
    ...
    def draw_box(self, box_coord, alpha=0.5, edge_color="g", line_style="-", output_image=None):
        """
        Args:
        ...
        output_image.ax.add_patch(
            mpl.patches.Rectangle(
                ...
                linewidth=linewidth * output_image.scale,
                alpha=alpha,
    ...
    def draw_rotated_box_with_label(
        self, rotated_box, alpha=0.5, edge_color="g", line_style="-", label=None, output_image=None
    ):
        """
        ...
        linewidth = self._default_font_size / (6 if area < _SMALL_OBJECT_AREA_THRESH * output_image.scale else 3)
        ...
        if label is not None:
            ...
            height_ratio = h / np.sqrt(output_image.height * output_image.width)
    ...
    def draw_circle(self, circle_coord, color, radius=3, output_image=None):
        """
        Args:
        ...
        if output_image is None:
            output_image = self.output
        x, y = circle_coord
        output_image.ax.add_patch(mpl.patches.Circle(circle_coord, radius=radius, fill=True, color=color))
        return output_image

    def draw_line(self, x_data, y_data, color, linestyle="-", linewidth=None, output_image=None):
        """
        Args:
    ...
    def draw_binary_mask(
        self, binary_mask, color=None, *, output_image=None, edge_color=None, text=None, alpha=0.5, area_threshold=10
    ):
        """
        Args:
        ...
        mask = GenericMask(binary_mask, output_image.height, output_image.width)
        ...
        if not mask.has_holes:
            ...
            for segment in mask.polygons:
                ...
                self.draw_polygon(segment, color=color, output_image=output_image, edge_color=edge_color, alpha=alpha)
        else:
            ...
            has_valid_segment = True
            output_image.ax.imshow(rgba, extent=(0, output_image.width, output_image.height, 0))

        if text is not None and has_valid_segment:
            lighter_color = self._change_color_brightness(color, brightness_factor=0.7)
            self._draw_text_in_mask(binary_mask, text, lighter_color, output_image=output_image)
        return output_image
    ...
    def draw_polygon(self, segment, color, output_image=None, edge_color=None, alpha=0.5):
        """
        Args:
        ...
        polygon = mpl.patches.Polygon(
            ...
            edgecolor=edge_color,
            linewidth=max(self._default_font_size // 15 * output_image.scale, 1),
        )
        output_image.ax.add_patch(polygon)
        return output_image
    ...
    def _create_grayscale_image(self, img, mask=None):
        """
        Create a grayscale version of the original image.
        The colors in masked area, if given, will be kept.
        """
        img_bw = img.astype("f4").mean(axis=2)
    ...
    def _draw_text_in_mask(self, binary_mask, text, color, output_image=None):
        """
        ...
        for cid in range(1, _num_cc):
            if cid == largest_component_id or stats[cid, -1] > _LARGE_MASK_AREA_THRESH:
                ...
                self.draw_text(text, center, color=color, output_image=output_image)
