...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prompt", type=str, help="user prompt for the task_name")
...
def _extract_segmentation(seg_outputs):
    ...
    mask_per_instance = _seg_to_numpy(seg_outputs['instances'].pred_masks)
    pred_classes = _seg_to_numpy(seg_outputs['instances'].pred_classes)

    return mask_per_instance, pred_classes
...
class XSamDemo:
    ...
    def _process_data_dict(self, data_dict):
        ...
        if isinstance(pil_image, Image.Image):
            ...
            vprompt_masks = []
            for idx, img in enumerate(pil_image):
                if self.image_processor is not None:
                    proc = self.image_processor
                    img_proc = getattr(proc, "image_processor", proc)
                    ...
                    image = img
                    if self.pad_image_to_square:
                        image = expand2square(img, tuple(int(x * 255) for x in img_proc.image_mean))
                    image = img_proc.preprocess(image, return_tensors="pt")["pixel_values"][0]
                    data_dict["pixel_values"].append(image)

                if self.extra_image_processor is not None:
                    seg_output = self.extra_image_processor.preprocess(img, condition_maps=data_dict["vprompt_masks"], return_tensors="pt")
                    data_dict["extra_pixel_values"].append(seg_output["pixel_values"][0])
                    data_dict["scaled_size"].append(tuple(seg_output["scaled_sizes"][0].tolist()))
                    vprompt_masks.append(seg_output.get("vprompt_masks", None))

            data_dict["pixel_values"] = torch.stack(data_dict["pixel_values"], dim=0) if len(data_dict["pixel_values"]) > 0 else None
            data_dict["extra_pixel_values"] = torch.stack(data_dict["extra_pixel_values"], dim=0) if len(data_dict["extra_pixel_values"]) > 0 else None
            data_dict["vprompt_masks"] = vprompt_masks if len(vprompt_masks) > 0 and vprompt_masks[0] is not None else None
    ...
    def run_on_image(self, image, prompt, task_name, segment_info, vprompt_masks=None, output_dir=None, **kwargs):
        ...
        masks, pred_ids = _extract_segmentation(seg_outputs)
        pred_labels = [segment_info[cls_id]["name"] for cls_id in pred_ids]

        masked_image, saved_path, masks_save_path = self.save_masked_image(
            image=image,
            masks=masks,
        ...
        return llm_input, generation_output, masked_image, pred_labels, saved_path, masks, masks_save_path, pred_ids

    def image_scaling_with_mask(self, image, mask, mode=None):
        ...
        gray = (0.299 * region_pixels[:, 0] + 0.587 * region_pixels[:, 1] + 0.114 * region_pixels[:, 2])
        ...
        if mode == 'outside':
            ...
            color = np.asarray(mode, dtype=np.uint8)
            color_tone = np.stack([gray * (color[0] / 255.0), gray * (color[1] / 255.0), gray * (color[2] / 255.0)], axis=-1)
            image_np[region] = color_tone

        return Image.fromarray(image_np) if input_is_pil else image_np, color_tone if mode != 'outside' else gray_tone


    def save_masked_image(self, image, masks, pred_labels, save_dir):
        save_dir = Path(save_dir)

        inverse_mask = np.ones_like(masks[0], dtype=bool)
        ...
        for i, mask in enumerate(masks):
            ...
            image, color_tone = self.image_scaling_with_mask(image, mask, mode=color)

            inverse_mask = inverse_mask & (mask == 0.)

            mask_save_path = os.path.splitext(save_dir)[0] + f'_mask_{i}_{pred_labels[i]}.png'
            Image.fromarray((mask * 255).astype(np.uint8), mode="L").save(mask_save_path)
            masks_save_path.append(mask_save_path)

        image, color_tone = self.image_scaling_with_mask(image, inverse_mask, mode='outside')

        png_path = str(save_dir)
        image.save(png_path)

        return image, png_path, masks_save_path
