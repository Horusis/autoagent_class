...
def _extract_segmentation(seg_outputs, mode='single'):
...
def save_masked_image(image, segmentation, segments_info, target_names, save_dir):
    seg = _seg_to_numpy(segmentation)
    save_dir = Path(save_dir)

    inverse_mask = np.ones_like(seg, dtype=bool)
    for info in segments_info:
        if info["name"] in target_names:
            target_id = int(info["id"])
            target_mask = (seg == target_id).astype(np.uint8)

            image = image_scaling_with_mask(image, target_mask, mode=info["color"])

            inverse_mask = inverse_mask & (seg != target_id)
...
class XSamDemo:
    ...
    def run_on_image(self, image, prompt, task_name, segment_info, target_names, vprompt_masks=None, output_dir=None, **kwargs):
        ...
        masked_image, saved_path = save_masked_image(
            image=image,
            segmentation=segmentation,
            segments_info=segment_info,
        ...
        seg_output = np.array(masked_image)

        return llm_input, generation_output, seg_output, saved_path, segmentations


    def run_on_batch(self, images, prompt, task_name, segment_info, target_names, vprompt_masks=None, output_dir=None, base_names=None, **kwargs):
        ...
        data_dict, data_samples = self._process_input_dict(data_dict_list)
        input_ids = data_dict["input_ids"]
        ...
        for seg_output in seg_outputs:
            seg_i = _extract_segmentation(seg_output, mode='single')
            if seg_i is not None:
                segmentation.append(seg_i)
        ...
        masked_images = []
        saved_paths = []
        for i, seg_i in enumerate(segmentation):
            m, s = save_masked_image(image=images[i], segmentation=seg_i, segments_info=segment_info, target_names=target_names, save_dir=os.path.join(save_dir, base_names[i]))
        ...
        seg_outputs = np.array(masked_images)

        return llm_input, generation_output, seg_outputs, saved_paths
